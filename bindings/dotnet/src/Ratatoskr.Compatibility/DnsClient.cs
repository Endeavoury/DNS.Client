...
public sealed class DnsClient
{
    ...
    public Task<DnsMessage> QueryAsync(string name, QuestionType type = QuestionType.A,
        QuestionClass @class = QuestionClass.IN, CancellationToken cancellationToken = default)
    {
        return NativeDnsAdapter.QueryAsync(servers, Timeout, Attempts, ThrowOnResponseError,
            name, type, @class, cancellationToken);
    }
    ...
    /// <summary>Queries the RFC 1035 IPv4 or RFC 3596 IPv6 reverse-DNS name.</summary>
    ...
    public static string GetReverseLookupName(IPAddress address)
    {
        ...
        throw new ArgumentException("Reverse DNS requires an IPv4 or IPv6 address.", nameof(address));
    }
