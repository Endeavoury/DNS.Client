...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Ratatoskr error {}: {}", self.code, self.message)
    }
}
...
) -> std::result::Result<DnsResult, Error> {
    if abi_version() != ABI_VERSION {
        return Err(Error {
            code: 9,
            message: "unsupported native ABI".into(),
        });
    }
    let name = CString::new(name).map_err(|_| Error {
        code: 2,
        message: "name contains NUL".into(),
    })?;
    let server = options
        .server
        .as_ref()
        ...
        .map_err(|_| Error {
            code: 2,
            message: "server contains NUL".into(),
        })?;
    unsafe {
        ...
        if context.is_null() {
            return Err(Error {
                code: 3,
                message: "could not allocate native context".into(),
            });
        }
        ...
        if count > 4096 {
            ...
            return Err(Error {
                code: 6,
                message: "native result exceeds safety limit".into(),
            });
        }
        ...
        for index in 0..count {
            ...
            records.push(Record {
                type_code: ratos_dns_record_type_code(record),
                section: ratos_dns_record_section(record),
                name: text(ratos_dns_record_name(record)),
                ttl: ratos_dns_record_ttl(record),
                text: text(ratos_dns_record_text(record)),
            });
        }
        let owned = DnsResult {
            query_name: text(ratos_dns_result_query_name(result)),
            query_type: ratos_dns_result_query_type(result),
            server: text(ratos_dns_result_server(result)),
            response_code: ratos_dns_result_rcode(result),
            records,
        };
        ratos_dns_result_destroy(result);
        ratos_context_destroy(context);
...
mod tests {
    ...
    fn defaults_are_stable() {
        let value = super::QueryOptions::default();
        assert_eq!((value.port, value.timeout_ms), (53, 5_000));
    }
}
